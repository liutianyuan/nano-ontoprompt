...
console.log('Buttons on /ontologies/new:');
for (const b of btns) {
  const t = await b.innerText().catch(() => '');
  const v = await b.isVisible().catch(() => false);
  if (t.trim()) console.log('[' + (v?'V':'H') + ']', JSON.stringify(t.trim().substring(0,60)));
}
const headings = await page.locator('h1,h2,h3').all();
for (const h of headings) console.log('H:', await h.innerText().catch(() => ''));
await browser.close();
