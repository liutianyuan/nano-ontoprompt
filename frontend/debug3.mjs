...
await ctx.addInitScript(t => { localStorage.setItem('token', t); localStorage.setItem('lang', 'zh'); }, token);
...
for (const b of btns) {
  ...
  if (t.trim()) console.log('[' + (v?'V':'H') + ']', JSON.stringify(t.trim().substring(0,50)));
}
await browser.close();
